...
void StorageSetError(const char *fileName, int lineNumber, const char *functionName, ErrorCode errorCode, ...)
{
    if (thrd != nullptr && thrd->error != nullptr) {
        va_list args;
        va_start(args, errorCode);
        thrd->error->SetErrorCommon(fileName, lineNumber, functionName, errorCode, args);
        va_end(args);
        thrd->error->SetErrorNodeId(g_storageInstance->GetGuc()->selfNodeId);
    }
}
                                   ...
                                   ErrorCode errorCode, ...)
{
    if (thrd != nullptr && thrd->error != nullptr) {
        ...
        thrd->error->SetErrorCommon(fileName, lineNumber, functionName, errorCode, args);
        va_end(args);
        thrd->error->SetErrorNodeId(nodeId);
    }
}
} /* namespace DSTORE */
