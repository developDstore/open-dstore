...
bool DataPage::IsHeapFastSkipEligibleByPageState(CRContext *crCtx) const
{
    if (g_storageInstance == nullptr || g_storageInstance->GetGuc() == nullptr ||
        !g_storageInstance->GetGuc()->enableHeapCrFastskipByPageCsn) {
        return false;
    }
    if (GetType() != PageType::HEAP_PAGE_TYPE) {
        return false;
    }
    if (crCtx == nullptr || crCtx->snapshot == nullptr) {
        return false;
    }
    if (crCtx->snapshot->GetSnapshotType() == SnapshotType::SNAPSHOT_DIRTY) {
        return false;
    }
    if (!IsHeapPageGenerationFirstInsertCsnValid()) {
        return false;
    }
    bool needCheckCurrentXidConflict = (crCtx->currentXid != INVALID_XID);
    if (!needCheckCurrentXidConflict) {
        return true;
    }
    for (OffsetNumber off = FIRST_ITEM_OFFSET_NUMBER; off <= GetMaxOffset(); off = OffsetNumberNext(off)) {
        ItemId *itemId = const_cast<DataPage *>(this)->GetItemIdPtr(off);
        if (itemId->IsUnused() || !itemId->IsNormal()) {
            continue;
        }
        TdId tupleTdId = const_cast<DataPage *>(this)->GetTupleTdId<HeapDiskTuple>(off);
        if (tupleTdId >= GetTdCount()) {
            continue;
        }
        TD *tupleTd = const_cast<DataPage *>(this)->GetTd(tupleTdId);
        if (tupleTd->GetXid() == crCtx->currentXid) {
            return false;
        }
    }
    return true;
...
void DataPage::BuildEmptyVisibleHeapCrPage()
{
    StorageAssert(GetType() == PageType::HEAP_PAGE_TYPE);
    for (OffsetNumber off = FIRST_ITEM_OFFSET_NUMBER; off <= GetMaxOffset(); off = OffsetNumberNext(off)) {
        ItemId *itemId = GetItemIdPtr(off);
        if (!itemId->IsUnused()) {
            itemId->SetUnused();
        }
    }
    ...
    SetHasFreeItemId();
    SetTuplePrunable(false);
    SetItemPrunable(false);
    HeapPage *heapPage = static_cast<HeapPage *>(this);
    heapPage->SetPotentialDelSize(0);
    heapPage->SetRecentDeadTupleMinCsn(INVALID_CSN);
}

HeapCrFastSkipDecision DataPage::TryFastSkipByPageGenerationFirstInsertXid(
    Transaction *transaction, CRContext *crCtx, CommitSeqNo &pageInitCsn)
{
    (void)transaction;
    pageInitCsn = INVALID_CSN;
    if (!IsHeapFastSkipEligibleByPageState(crCtx)) {
        return HeapCrFastSkipDecision::NOT_ELIGIBLE;
    }

    pageInitCsn = GetHeapPageGenerationFirstInsertCsn();
    if (pageInitCsn == INVALID_CSN) {
        return HeapCrFastSkipDecision::CONTINUE_NORMAL;
    }
    if (crCtx->snapshot->GetCsn() > pageInitCsn) {
        return HeapCrFastSkipDecision::CONTINUE_NORMAL;
    }

    BuildEmptyVisibleHeapCrPage();
    crCtx->useLocalCr = true;
    crCtx->pageMaxCsn = pageInitCsn;
    return HeapCrFastSkipDecision::SKIP_ALL_INVISIBLE;
}
...
#endif
    if (GetType() == PageType::HEAP_PAGE_TYPE && btrUndoContext == nullptr) {
        CommitSeqNo pageInitCsn = INVALID_CSN;
        HeapCrFastSkipDecision decision =
            TryFastSkipByPageGenerationFirstInsertXid(transaction, crCtx, pageInitCsn);
        if (decision == HeapCrFastSkipDecision::SKIP_ALL_INVISIBLE) {
            return DSTORE_SUCC;
        }
    }
    /*
